...
function condaactivate -d 'Activate a conda environment' -a cenv
  if test -z $cenv
  ...
  set -xg DEFAULT_LD_LIBRARY_PATH $LD_LIBRARY_PATH

  # put the condabin directory at the front of the PATH
  set -xg PATH $condabin $PATH
  set -xg LD_LIBRARY_PATH $HOME/miniconda3/lib $LD_LIBRARY_PATH
...
function deactivate -d 'Deactivate a conda environment'
  if set -q __CONDA_ENV_ACTIVE
    ...
    set -e DEFAULT_PATH
    set -xg LD_LIBRARY_PATH DEFAULT_LD_LIBRARY_PATH
    set -e DEFAULT_LD_LIBRARY_PATH
